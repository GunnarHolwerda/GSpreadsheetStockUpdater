"""
This script it used to copy the end portfolio value to another spreadsheet so that it can be analyzed
"""
...
parser.add_argument("spreadsheet_key", help="The spreadsheet key of the Google Spreadsheet to update")
...
# If a spreadsheet title was specified load by title, else default to the second worksheet
if args.portfolio_spreadsheet:
    value_worksheet = spreadsheet.worksheet(args.spreadsheet_title)
else:
    value_worksheet = spreadsheet.get_worksheet(1)

if args.value_spreadsheet:
    portfolio_worksheet = spreadsheet.worksheet(args.value_spreadsheet)
else:
...
value_worksheet.update_cell(row_of_cell_to_update, args.value_col, end_of_day_value)
